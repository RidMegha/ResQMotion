...
public class MainActivity extends AppCompatActivity implements SensorEventListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        familyButton          = findViewById(R.id.familyButton);   // PHASE 4
        ...
        ensureInviteCodeExists();   // PHASE 4: generate invite code if missing
    ...
    // ═════════════════════════════════════════════════════════
    //  PHASE 4 – ENSURE INVITE CODE EXISTS
    // ═════════════════════════════════════════════════════════
    /**
     * When a user first logs in on a new device / after registration,
     * they may not have an invite_code yet.
     * This generates and saves one if missing.
     */
    private void ensureInviteCodeExists() {
        ...
        mDatabase.child("users").child(user.getUid()).child("invite_code").get()
                .addOnSuccessListener(snapshot -> {
                    if (!snapshot.exists() || snapshot.getValue() == null) {
                        String code = generateInviteCode(user.getUid());
    ...
    private void uploadCurrentStatus() {
        ...
        if (lastSyncedActivity.isEmpty()) return;
    ...
    private void uploadFallToFirebase(float probability) {
        ...
        alertStatus.put("fall_alert", true);
    ...
    private int getBatteryLevel() {
        ...
        } catch (Exception e) { Log.w(TAG, "Battery unavailable"); }
        return -1;
    }
    ...
    private void onFallDetected(float probability) {
        ...
        alertHandler.postDelayed(() -> { if (fallDetected) sendEmergencySMS(); }, ALERT_DELAY_MS);
    }
    ...
    private void updateButtonStates() {
        ...
        if (familyButton != null) familyButton.setEnabled(!isMonitoring);
        ...
        if (familyButton != null) familyButton.setAlpha(isMonitoring ? 0.5f : 1.0f);
    }
    ...
    private void loadUserProfile() {
        ...
        } catch (Exception e) { userProfile = null; }
    }
    ...
    private void openProfileView() {
        startActivityForResult(new Intent(this, ProfileSetupActivity.class), PROFILE_REQUEST_CODE);
    }
    ...
    private void loadEmergencyContacts() {
        try {
            ...
            } else emergencyContacts = new ArrayList<>();
        } catch (Exception e) { emergencyContacts = new ArrayList<>(); }
    }
    ...
    private void updateEmergencyContactsDisplay() {
        ...
        if (emergencyContacts.isEmpty()) {
            ...
            for (int i = 0; i < emergencyContacts.size(); i++) {
                ...
                sb.append("📱 ").append(c.getName()).append(" (").append(c.getPhone()).append(")");
    ...
    private void openContactsManager() {
        startActivityForResult(new Intent(this, EmergencyContactsActivity.class), CONTACTS_REQUEST_CODE);
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == CONTACTS_REQUEST_CODE && resultCode == RESULT_OK) {
            loadEmergencyContacts(); updateEmergencyContactsDisplay();
            ...
            loadUserProfile(); updateProfileDisplay();
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        try {
            if (event.sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
                ...
                synchronized (ax) { ax.add(event.values[0]); ay.add(event.values[1]); az.add(event.values[2]); }
                final float magnitude = calculateMagnitude(lastAccel);
                runOnUiThread(() -> {
                    if (sensorDataText != null)
                        sensorDataText.setText(String.format("Accel: %.2f m/s² | Samples: %d",
                                magnitude, ax.size()));
                });
                ...
                synchronized (gx) { gx.add(event.values[0]); gy.add(event.values[1]); gz.add(event.values[2]); }
            }
            trimBuffers();
            ...
            if (now - lastInferenceTime > INFERENCE_INTERVAL_MS && minSize >= KUHAR_WINDOW) {
                ...
                new Thread(() -> {
                    ...
                    } catch (Exception e) { Log.e(TAG, "Inference error: " + e.getMessage(), e); }
                }).start();
            }
        } catch (Exception e) { Log.e(TAG, "Sensor error: " + e.getMessage(), e); }
    }
    ...
    private String simplifyActivity(String activity) {
        ...
        if (activity.contains("Standing") || activity.equals("Stand to Sit") || activity.equals("Stand to Lie")) return "Standing";
        if (activity.contains("Sitting")  || activity.equals("Sit to Stand") || activity.equals("Sit to Lie"))   return "Sitting";
        if (activity.contains("Lying")    || activity.equals("Lie to Sit")   || activity.equals("Lie to Stand")) return "Lying Down";
        if (activity.contains("Turning")) return "Turning";
        return activity;
    }

    private float normalizeValueKuhar(float value) { return (value - TRAINING_MEAN) / TRAINING_STD; }
    ...
    private void runKuhar() {
        ...
        try {
            ...
            for (int i = 1; i < 18; i++) { if (output[0][i] > max) { max = output[0][i]; best = i; } }
            ...
            if (displayActivity.equals(lastDisplayedActivity)) activityConfirmCount++;
            else { lastDisplayedActivity = displayActivity; activityConfirmCount = 1; }
        ...
        } catch (Exception e) { Log.e(TAG, "Kuhar error: " + e.getMessage(), e); }
    }

    private String determineStaticActivity() {
        float absX = Math.abs(lastAccel[0]), absY = Math.abs(lastAccel[1]), absZ = Math.abs(lastAccel[2]);
        ...
        try {
            ...
            synchronized (ax) {
                ...
                for (int i = 0; i < SISFALL_WINDOW; i++) {
                    int idx = offset + i;
                    input[0][i][0] = ax.get(idx); input[0][i][1] = ay.get(idx); input[0][i][2] = az.get(idx);
                }
            }
            synchronized (gx) {
                ...
                for (int i = 0; i < SISFALL_WINDOW; i++) {
                    int idx = offset + i;
                    input[0][i][3] = gx.get(idx); input[0][i][4] = gy.get(idx); input[0][i][5] = gz.get(idx);
                }
            }
            ...
            } else fallConfirmCount = 0;
        } catch (Exception e) { Log.e(TAG, "Sisfall error: " + e.getMessage(), e); }
    }
    ...
    private void sendEmergencySMS() {
        if (!checkPermissions()) { Toast.makeText(this, "SMS permission denied", Toast.LENGTH_SHORT).show(); return; }
        if (emergencyContacts.isEmpty()) { Toast.makeText(this, "No emergency contacts", Toast.LENGTH_SHORT).show(); return; }
        try {
            ...
            runOnUiThread(() -> {
                Toast.makeText(this,
                        failed == 0 ? "✓ SMS sent to " + sent + " contact(s)"
            ...
            fallDetected = false; fallConfirmCount = 0;
            emergencyButton.setEnabled(false);
        } catch (Exception e) {
    ...
    private void trimBuffers() {
        ...
        synchronized (ax) { while (ax.size() > max) { ax.remove(0); ay.remove(0); az.remove(0); } }
        synchronized (gx) { while (gx.size() > max) { gx.remove(0); gy.remove(0); gz.remove(0); } }
    }
    ...
    public void onRequestPermissionsResult(int requestCode,
        ...
        if (requestCode == PERMISSION_REQ_CODE) {
            ...
            Toast.makeText(this, all ? "Permissions granted" : "Some permissions denied",
